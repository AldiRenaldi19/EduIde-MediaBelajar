...
<body class="min-h-screen flex items-center justify-center p-6 relative">
    ...
    <div class="fixed top-8 left-8 z-50">
        <a href="{{ url('/') }}" class="group flex items-center gap-3 bg-white/5 hover:bg-white/10 border border-white/10 px-6 py-3 rounded-2xl transition-all backdrop-blur-md">
            ...
            <span class="text-[10px] font-bold uppercase tracking-widest text-gray-300">Batal</span>
        </a>
    </div>
    ...
    <div class="w-full max-w-lg mt-12 mb-12 fade-in">
        ...
        <div class="glass-auth rounded-[40px] p-10 md:p-12 relative overflow-hidden">
                ...
                <div class="mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-2xl">
                    <ul class="list-none">
                        @foreach ($errors->all() as $error)
                            <li class="text-[11px] text-red-400 leading-relaxed">• {{ $error }}</li>
                        @endforeach
                    </ul>
                </div>
            @endif
            ...
            <div class="relative my-10">
                ...
                <div class="relative flex justify-center text-[9px] uppercase font-bold tracking-[0.3em]"><span class="bg-[#0b0b11] px-4 text-gray-600">Atau Daftar Dengan</span></div>
            </div>
            ...
            <a href="{{ route('google.login') }}" class="w-full flex items-center justify-center gap-3 bg-white/5 hover:bg-white/10 border border-white/10 py-4 rounded-2xl transition-all group">
                ...
                <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest group-hover:text-white transition-colors">Google Account</span>
            </a>
        </div>

        <p class="mt-8 text-center text-[10px] text-gray-500 font-bold tracking-widest uppercase mb-12">
            Sudah punya akun? 
            <a href="{{ route('login') }}" class="text-indigo-400 hover:text-indigo-300 transition-colors ml-1 border-b border-indigo-400/20">Masuk Sekarang</a>
        </p>
    </div>
</body>
</html>