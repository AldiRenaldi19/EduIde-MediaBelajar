...
<body class="min-h-screen relative" style="background-color: var(--bg-page); color: var(--text-main);">
    ...
    <div class="glow-blob top-0 left-0 bg-indigo-600/10"></div>
    ...
    <div class="container mx-auto px-6 py-24 md:py-32">
        <div class="max-w-2xl mx-auto">
            ...
            <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data" class="glass-card rounded-[32px] p-8 md:p-12 border border-white/10 mb-8">
                ...
                {{-- Name Field --}}
                <div class="mb-8">
                    <label for="name" class="block text-xs font-black uppercase tracking-widest text-indigo-400/80 mb-3 ml-1">Nama Lengkap</label>
                    <input type="text" name="name" id="name" value="{{ old('name', auth()->user()->name) }}" 
                           class="input-glass w-full px-6 py-4 rounded-2xl text-sm @error('name') border-red-500 @enderror" 
                ...
                {{-- Email Field --}}
                <div class="mb-8">
                    <label for="email" class="block text-xs font-black uppercase tracking-widest text-indigo-400/80 mb-3 ml-1">Email</label>
                    <input type="email" name="email" id="email" value="{{ old('email', auth()->user()->email) }}" 
                           class="input-glass w-full px-6 py-4 rounded-2xl text-sm @error('email') border-red-500 @enderror" 
